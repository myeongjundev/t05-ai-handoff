...
`FINAL_COMPARISON.md`는 원본 증거로 보존되어 있으며, 현재 아래 네 항목은 확정본에
반영하기 전에 수정 또는 사용자 판단이 필요하다.

1. `2026-08-31 12:0x`를 실제 완료 시각으로 바꾼다.
2. “사람이 브라우저로 앱을 직접 조작”했다는 문구를 실제 수행 방식인 Codex의 로컬
   Playwright 검증과 GitHub Actions 검증으로 바로잡는다.
3. 기존 e2e는 31건이 아니라 **30건**이다. 최종 38건은 기존 30건과 T05에서 추가한
   `compare.e2e.js` 8건으로 구성된다.
4. `다음 과제의 선택 기준` 3개와 `AI 3줄`은 사용자의 최종 판단을 적는 자리라 아직
   초안 문구 또는 `미기입` 상태다.
